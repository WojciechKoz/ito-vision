...
class InDI(IterativeRefinementMethod):
    def __init__(
        self,
        scheduler: Scheduler,
        tau: float = 0.06,
    ...
    def g(self, t: torch.Tensor) -> torch.Tensor:
        ...
        return self.tau * torch.sqrt(2 * (1 - phi_t) * beta_t)
    ...
    def transition_lambda_y(
        ...
        phi_st: torch.Tensor = self.scheduler.geometric_integral(
            t, exponent=-1, lower_bound=s
        )
    ...
    def transition_std(
        ...
        return self.tau * torch.sqrt(1.0 - phi_st**2 + 2 * phi_st * phi_t - 2 * phi_t)
